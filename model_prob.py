# api_client.py
# A tiny client to call a remote /classify API with (image_path, task_id) -> probability in [0,1].
...
class ClassifyAPIClient:
    ...
    Simple client for a remote Flask API:
        ...
        JSON request: { "task_id": int, "image": "<base64 string>" }
        JSON response: { "complete": <probability in [0,1]>, ... }

    Usage:
        client = ClassifyAPIClient(api_url="http://host:8000", api_key=None)
        prob = client.check(image_path, task_id=3)  # returns float in [0,1]
    """
    ...
    #     """
    #     Convert a variety of types to a probability in [0,1].
    #     Accepted:
    #       - float/int: clamped to [0,1]
    #       - bool: True->1.0, False->0.0
    #       - str: parsed to float, then clamped to [0,1]
    #     Raises APIClientError if the value cannot be interpreted as a probability.
    #     """
    ...
    #         except Exception:
    #             pass
    #     raise APIClientError(f"Unsupported 'complete' type for probability: {type(value)} | value={value!r}")

    # ------------- public API -------------

    def check(self, image_path: str, task_id: int) -> float:
        """
        Returns a probability in [0,1] for 'complete' according to the server.
...
def check_task_complete(
    ...
    *,
    ...
    """
    One-shot convenience function: return a probability in [0,1].
    ...
    -------
    float
        Probability in [0,1] from the server's 'complete' field.
